...
> **CANONICAL companion to `Fractal_Node_and_Event_Schema_v0.6.md` (Domain KG).** Copy-paste starting points for writing nodes and log lines by hand, at zero infrastructure. Comments explain each field; delete them in real nodes. This template is a derived convenience of the schema — the schema is the source of truth.

**Fractal_Node_Template** · **Version:** 0.5 · **Status:** Ratified (2026-08-15, in-conversation per C-033, per Protocol v0.31) · **Reviewed By:** Max (2026-08-14, per v0.17 — last full review; v0.5 is a pointer-currency reissue on Max's call, not a re-review) · **Updated:** 2026-08-15 · **Domain:** KG · **Author:** Claude · **Parent:** Fractal Node & Event Schema v0.6 · **Supersedes:** Fractal_Node_Template_v0.4.md

> **v0.5 (pointer-currency reissue, arc 1):** companion/parent pointers cured v0.5 → **v0.6** — the Schema reissued to v0.6 on 2026-08-14 (C-068–C-070) and the C-059 coupling row was walked as *"Template v0.4 coupling checked — unchanged"* (Protocol v0.24), which was right about **content** (v0.6 made no format change) and silent about **pointers**. The gap surfaced when `genesis.py` first birthed an instance: in FRACTAL the stale pointer is invisible (v0.5 exists as history), but in a newborn instance the tier-0 set holds only v0.6, so the pointer named a document that was not there — `check_versions.py` reported it as a hard error. **A currency defect that is cosmetic in FRACTAL becomes structural the moment the kernel is copied** (the C-080 lesson, in a second file). Recorded consequence: the C-059 Schema→Template coupling walk must check pointers, not only content.

> **v0.4 (currency reissue, loose-ends pass):** companion/parent pointers cured v0.3 → v0.5 (scan S2-1.5); the §C example appends to `_events/part-0001.jsonl` — the C-055 one-file log (the bootstrap monthly name is retired). Schema reissues now oblige a same-pass Template review (C-059 coupling). Carried from v0.3 (workstream D): the reparent-by-redirect worked example (§E) — re-mint on the same subject + `alias` of `kind: redirect`, per C-045; alias kinds `label`/`route`/`redirect` noted where they appear; facet tokens now resolve to minted `FACET.*` concepts (C-043) — nothing changes in how you *write* a `facet:` field. Root codes are globally unique across facets (C-044): check existing roots (`grep '"parent": null' _events/*.jsonl`) before minting a new root.
...
**Sources:** Fractal Node & Event Schema v0.6; Fractal Knowledge Path Foundation v0.1; Loose-Ends Scan #2 (S2-1.5), per Protocol v0.17; the first birthed instance's checker output (arc 1, per Protocol v0.31).
**Revision history:** v0.1 (2026-08-07) initial templates + worked example. · v0.2 (2026-08-12) agent codes kind-branched per C-026; valid coherent ULIDs throughout; `EVT` in the TYPE list; `facet:` in the TOP example; entity-target placement syntax (C-040/C-041, per Protocol v0.9). · v0.3 (2026-08-13) reparent-by-redirect worked example (§E, C-045); alias-kind annotations (label/route/redirect); facet-token and global-root notes (C-043/C-044, per Protocol v0.10). · v0.4 (2026-08-14) currency reissue (scan S2-1.5, per Protocol v0.17): companion/parent → Schema v0.5; §C log path → `part-0001.jsonl` (C-055); C-059 Schema→Template coupling noted. · v0.5 (2026-08-15) pointer-currency reissue (arc 1 of C-078, per Protocol v0.31): companion/parent/Sources → Schema **v0.6**, curing a stale pointer that Protocol v0.24's content-only coupling walk left behind and that the first birthed instance exposed as a hard error.
