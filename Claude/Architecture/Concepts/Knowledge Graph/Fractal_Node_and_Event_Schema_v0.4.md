...
**Fractal_Node_and_Event_Schema** · **Version:** 0.4 · **Status:** Ratified (2026-08-13, in-conversation per C-033; concurrency pins by C-049–C-050) · **Updated:** 2026-08-13 · **Domain:** KG · **Author:** Claude · **Parent:** Fractal Knowledge Path Foundation v0.1 · **Supersedes:** Fractal_Node_and_Event_Schema_v0.3.md

**Decision status legend:** *Principle* (accepted, load-bearing) · *Working Decision* (accepted) · *Deferred* · *TBD*.

> **Why v0.4 exists (version-bump discipline, C-040).** v0.3 completed the facet layer but carried OQ-19 open: “multi-person safety” was a claimed property with every enabling mechanism deferred (scan 4.1/8.2), so the store ran under a single-writer caveat. This reissue closes it. **Identity is collision-proof and codes resolve deterministically** — when a merge lands two mints of one code, the first mint wins (by `ts`, event-ULID tie-break) and the cure is append-only (C-049); **root mints carry ceremony** in the C-044 global namespace — an explicit flag and a recorded call (C-049); and **verification is a duty with a canonical tool** — the repo-resident `verify.py` must pass at every store-touching C-037 boundary, with `mint.py` making local duplicates impossible at the source (C-050). The single-writer caveat is retired.
...
- **Physical layout:** the log is **partitioned** into time-bucketed files `_events/<bucket>.jsonl`. "One log" is conceptual; partitioning keeps files small and makes git merges of concurrent appends near-conflict-free — *textually*; semantic clashes (two writers minting one code) are invisible to git and are caught by the §5.5 ritual (C-049/C-050). **The partition interval is TBD** (C-023 / OQ-3): monthly (`_events/2026-08.jsonl`) is a candidate shown for illustration, not a fixed decision — the bracket size is chosen later from observed volume, which the git history now accrues.
...
*TBD:* whether `time` placements should auto-derive from `created` (OQ-18 — non-blocking); a compact binary/columnar projection for very large logs; the log partition interval (OQ-3). *Deferred:* the `place`/`trace`/`query-by-facet` MCP index and the Galaxy UI (build only on observed need, C-021). *Resolved in v0.4 (2026-08-13):* the concurrency & collision policy — first-mint-wins, append-only cure, root-mint ceremony (C-049); the verification ritual + repo-resident tools (C-050); OQ-19 closed, the single-writer caveat retired. *Resolved in v0.3 (2026-08-13):* facet minting under the `FACET` root + meta-facet token binding (C-043); global root-uniqueness (C-044); reparent/redirect semantics + the `redirect` alias kind + label/route/redirect fold rules (C-045); rule 7 exercised via live route bindings (C-046). *Resolved in v0.2 (2026-08-12):* the ULID profile, `content_hash` input, `revise` referent, `actor` semantics, TOP `facet:` field, entity-target placements (C-040); the C-015 scheme itself (C-039, closing OQ-14); the identity bridge to canonical documents (C-041).

## 7. Decisions captured

Carried from v0.1–v0.3: **C-022** (node file), **C-023** (event log; interval TBD), **C-024** (log-canonical), **C-025** (supersede + commit granularity; boundary = the git commit per C-037), **C-026** (facet registry + vocabularies), **C-027** (allocation grammar vs style), **C-039** (C-015 confirmed), **C-040** (format pins), **C-041** (identity bridge), **C-042** (canonical corpus minted); **C-043–C-046** (facet layer: minted facets, global roots, redirect semantics, live route bindings — see v0.3). New in v0.4 (recorded in Governance Protocol — Claude Series v0.13):

- **C-049** — **Concurrency & collision policy.** Identity cannot collide (ULID randomness); codes resolve **first-mint-wins** (earlier `ts`, event-ULID tie-break) when a merge lands two mints of one code; cure is mandatory and append-only — re-mint the losing subject under a fresh code and re-point its intended placements (verifier-listed); root mints from the pin forward require the guard's explicit flag plus a `note` naming whose call it was; the single-writer caveat on C-032 is retired. Resolves OQ-19 (with C-050).
- **C-050** — **Verification ritual + tools canonized.** `verify.py` (fold verifier) and `mint.py` (mint guard) are repo-resident in the store, stdlib-only, C-006-exception code artifacts, not DOC-minted. Duty: `verify.py` runs and passes before every store-touching commit and after every merge (the C-037 boundary). The README's trace recipes are corrected alongside (scan 5.3).
