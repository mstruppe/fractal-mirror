...
**Fractal_Node_and_Event_Schema** · **Version:** 0.7 · **Status:** Ratified (2026-08-20, in-conversation per C-033 — Max's "full slate ratified") · **Updated:** 2026-08-20 · **Domain:** KG · **Author:** Claude · **Parent:** Fractal Knowledge Path Foundation v0.1 · **Supersedes:** Fractal_Node_and_Event_Schema_v0.6.md · **Companion:** Fractal Navigation Contract v0.2 (read side, C-070) · **Document ID:** DOC-01M0G6V1PC9JR1RT4RFTHHRC96

**Reviewed By:** Claude (AGENT.AI.CLAUDE) — full conformance review against the live store, 2026-08-20: the fold verifier green at 151 nodes / 977 events; all eight domain routes checked live in the log; the §5.1 facet table folded clean against the six `FACET.*` mints; the ULID, hash, and collision clauses re-read against `verify.py`'s invariant suite · **Ratified by:** Max Struppe (owner — C-033 in-conversation; kernel tier, auto-ratified as shipped per C-133)

**Decision status legend:** *Principle* (accepted, load-bearing) · *Working Decision* (accepted) · *Deferred* · *TBD*.

> **Why v0.7 exists (version-bump discipline, C-040).** A **currency-and-review reissue, no format change** — the release boundary its substantive vehicle (beta-0.7, Max's scope decision of 2026-08-20). (1) The header gains the **`Reviewed By` field** (the S3-2.2 cure — S2-2.2's forward commitment finally landing; the field's first instance is also **C-133's first exercise**: the reviewed-by-agent · ratified-by-owner split made visible). (2) §5.3 rule 7's parenthetical is cured to the historical truth (the S5-2.4 cure): all eight routes have been live since 2026-08-13 (C-053). (3) Two currency touches from the review pass itself: the §5.1 topic row names the live domain roots, and §5.5 records that the store's tool roster grew past the original pair (the README the roster of record). **C-059 Schema→Template coupling checked at this reissue: Template v0.5 unchanged** — no front-matter field, verb, or allocation rule moved.
...
It does **not** specify how the graph is *read*: entry, hops, trace semantics, ranking, and the read-verb surface are fixed by the companion **`Fractal_Navigation_Contract_v0.1.md`** (C-068–C-070). Implementations of that contract — the index/MCP server, the Galaxy UI — remain Deferred per the Foundation and C-021.
...
- **Versioned specifications: DOC identity mirrors file identity (C-061, per Protocol v0.19 — documented in-spec from v0.6, discharging the 2026-08-14 standing note).** For versioned canonical artifacts (this Schema, the Template, the Navigation Contract, …), **every version file carries its own `DOC-…` identity** with a `route` alias binding its full name (`Fractal_<Name>_vX.Y`); the **series** is a separate living pointer node holding the **versionless** alias (`Fractal_<Name>`), which always resolves to the newest version. A spec reissue is therefore **one series `revise` + one per-version mint** — exactly the treatment the Governance Protocols always had. Superseded versions stay in place as history, each retaining its own identity forever.
...
`weight ∈ [0,1]` — how central the placement is. `1.0` = definitive / core; lower = looser / peripheral. It is what distinguishes *central to* a topic from *merely tagged*, and lets a trace rank neighbours. Default when omitted: `1.0`. *(The read side takes weight seriously: it is the ranking primitive of the Navigation Contract — landmark ranking at entry, and the path-score product in a trace.)*
...
Placements are written **outward** (subject → target). The reverse view (target → its subjects) is not stored in nodes; it is recovered by folding the log (or, later, from the index's inverted table). At zero infra you find "everything placed at `PHY.QM.QFT`" by grepping the log / node front-matter for that code — after first checking the redirect list if the concept has been reparented (§5.3 rule 6; store README). *(The read side names this recovery the `in` hop and specifies its semantics — Navigation Contract §2; the inverted view is the load-bearing read behind the entry rule there, and the sharpened trigger for the index tier.)*
...
| `topic` | `FACET.TOPIC` | subject matter | domain roots — live: `KG`, `GOV`, `GBL`, `CTX`, `AST`; open-ended (`PHY`, `ECON`, `BIO`, … as content arrives) |
...
7. **Routing codes derive from identity (exercised, C-046; completed by C-053).** Index-style routes (`GBL`, `KG`, …) are `alias` bindings (`kind: route`) over concept ids — projections of placements, never independent strings. **All eight of the Context Index's domain routes (`KG`, `GOV`, `GBL`, `CTX`, `AST`, `ONT`, `WS`, `PROT`) are live bindings in the store since 2026-08-13** (the first four per C-046, the remainder completed the same day per C-053); the Index's routing table is a projection of those bindings and cannot drift from them. Facet tokens use the same mechanism (§5.1, C-043). *(v0.1–v0.6 carried a "follow when workstream F settles" parenthetical for the second four — stale from the day before v0.6's issue; cured here, S5-2.4.)*
...
*(Roster note, v0.7 — a projection, not a rule change: the store's tool population has grown well past this original C-050 pair — eight canonized tools at this writing, each ratified by its own decision (C-050…C-118); the store `README.md` is the roster of record. The two named above remain this section's subject: the verification ritual and the mint guard, unchanged.)*
...
*TBD:* whether `time` placements should auto-derive from `created` (OQ-18 — non-blocking); a compact binary/columnar projection for very large logs. *Resolved in v0.6 (2026-08-14):* the read-side contract — entry, hops, trace, ranking, history — is fixed by the companion **Navigation Contract v0.1** (C-068–C-070); its implementations (the `place`/`trace`/`query-by-facet` MCP index, the Galaxy UI) remain *Deferred* per C-021, with the index tier's trigger sharpened there (§6: inverted-view reads becoming routine). *Resolved in v0.5 (2026-08-13):* the event-log physical layout — one append-only file, 50,000-event advisory tripwire, roll-by-ceremony (C-055, closing OQ-3; interval-based brackets retired unchosen). *Resolved in v0.4 (2026-08-13):* the concurrency & collision policy — first-mint-wins, append-only cure, root-mint ceremony (C-049); the verification ritual + repo-resident tools (C-050); OQ-19 closed, the single-writer caveat retired. *Resolved in v0.3 (2026-08-13):* facet minting under the `FACET` root + meta-facet token binding (C-043); global root-uniqueness (C-044); reparent/redirect semantics + the `redirect` alias kind + label/route/redirect fold rules (C-045); rule 7 exercised via live route bindings (C-046). *Resolved in v0.2 (2026-08-12):* the ULID profile, `content_hash` input, `revise` referent, `actor` semantics, TOP `facet:` field, entity-target placements (C-040); the C-015 scheme itself (C-039, closing OQ-14); the identity bridge to canonical documents (C-041).

## 7. Decisions captured

Carried from v0.1–v0.5: **C-022** (node file), **C-023** (event log; physical layout fixed by C-055), **C-024** (log-canonical), **C-025** (supersede + commit granularity; boundary = the git commit per C-037), **C-026** (facet registry + vocabularies), **C-027** (allocation grammar vs style), **C-039** (C-015 confirmed), **C-040** (format pins), **C-041** (identity bridge), **C-042** (canonical corpus minted); **C-043–C-046** (facet layer — see v0.3); **C-049–C-050** (concurrency policy + verification ritual — see v0.4); **C-055** (event-log physical layout — see v0.5).

**New in v0.6: no format decisions.** The reissue registers the read-side companion (**C-070** — the Navigation Contract, where C-068/C-069 live) and discharges the C-061 documentation note (§2.2) — both recorded in the Governance Protocol at this conversation's close.

**New in v0.7: no format decisions.** A currency-and-review reissue at the beta-0.7 boundary: the `Reviewed By` field lands (S3-2.2 — C-133's first exercise), rule 7's route parenthetical is cured (S5-2.4), and the two review-pass currency touches (§5.1 topic roots; the §5.5 roster note). Recorded in the Governance Protocol at this session's close.

---

**Refresh triggers:** a change to the two-relation core, the canonical-source rule, the verb set, the role vocabulary, the facet-minting or code-allocation rules, the concurrency/collision policy, the verification duty, the ULID profile, or the hash definition. *(The §5.1 facet table refreshes as a projection whenever a facet is minted — that alone is not a version bump. Read-side changes refresh the Navigation Contract, never this document — C-070.)*
**Sources:** Fractal Knowledge Path Foundation v0.1; Fractal Node & Event Schema v0.1–v0.6 (superseded); Fractal Navigation Contract v0.2 (companion, C-070); Governance Protocol — Claude v0.4, v0.6, v0.8–v0.10, v0.13, v0.15 & v0.19 (C-061); Decision Register (stamp inside, C-012 — the S3-2.2/S5-2.4 ledger rows, C-053, C-133); the 2026-08-14 conversation (KG navigation architecture — Max's calls: entry as query shape, weight-product ranking, dedicated contract doc with Schema linking); the 2026-08-20 review session (the beta-0.7 scope decision + the C-133 ratification, protocol at that session's close).
**Revision history:** v0.1 (2026-08-07) initial node & event format specification; (2026-08-12) in-place revision at facet-registry acceptance — retroactively recognized as a C-005 violation (scan 1.2). · v0.2 (2026-08-12) reissue under version-bump discipline: C-015 confirmed (C-039); ULID profile, `content_hash`, `revise` referent, `actor` semantics, TOP `facet:`, entity-target placements pinned (C-040); identity bridge (C-041). · v0.3 (2026-08-13) facet-layer completion (workstream D): facets minted under `FACET` (C-043); global root-uniqueness (C-044); redirect semantics + alias-kind fold rules (C-045); route binding exercised (C-046). · v0.4 (2026-08-13) multi-writer safety (workstream E): concurrency & collision policy — first-mint-wins, append-only cure, root ceremony (C-049); verification ritual + repo-resident tools `verify.py`/`mint.py` (C-050); OQ-19 closed, single-writer caveat retired. · v0.5 (2026-08-13) event-log layout (OQ-3 resolution): one append-only file, 50k-event advisory tripwire, roll-by-ceremony (C-055); monthly illustration retired; `mint.py` appends to the single active partition. · v0.6 (2026-08-14) read-side registration: Navigation Contract v0.1 registered as companion (C-070; cross-pointers §0/§3.4/§3.6/§4.4); C-061 spec-version identity documented in-spec (§2.2, standing note discharged); no format change; Template v0.4 coupling checked — unchanged. · v0.7 (2026-08-20) **the currency-and-review reissue (the beta-0.7 boundary its vehicle — Max's scope decision; the spec-reissue pair's cure):** the `Reviewed By` header field lands (S3-2.2 — the S2-2.2 forward commitment discharged; C-133's first exercise, the reviewed-by-agent · ratified-by-owner split); §5.3 rule 7 cured to the eight-routes truth (S5-2.4 — C-053's completion had predated v0.6 by a day); §5.1 topic row names the live roots; §5.5 gains the roster note (eight tools, README of record); no format change; Template v0.5 coupling checked — unchanged.
