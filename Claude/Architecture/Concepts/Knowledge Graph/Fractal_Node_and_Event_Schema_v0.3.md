...
**Fractal_Node_and_Event_Schema** · **Version:** 0.3 · **Status:** Ratified (2026-08-13, in-conversation per C-033; facet-layer pins by C-043–C-046) · **Updated:** 2026-08-13 · **Domain:** KG · **Author:** Claude · **Parent:** Fractal Knowledge Path Foundation v0.1 · **Supersedes:** Fractal_Node_and_Event_Schema_v0.2.md

> **SUPERSEDED (2026-08-13):** reissued as `Fractal_Node_and_Event_Schema_v0.4.md` (C-049–C-050, per Protocol v0.13). Retained as history under C-012; do not author against this version.

**Decision status legend:** *Principle* (accepted, load-bearing) · *Working Decision* (accepted) · *Deferred* · *TBD*.

> **Why v0.3 exists (version-bump discipline, C-040).** v0.2 closed the format's identity pins but left the facet layer's own foundations queued as workstream D (scan 3.3/3.4/8.1). This reissue resolves them: **facets are themselves minted** as concepts under a `FACET` root in a meta-facet, so the facet set escapes the hand-authored control table (C-043); **root-code uniqueness is global** across facets (C-044); **reparenting has pinned redirect semantics** — re-mint on the same subject + an explicit `redirect` alias, with the fold resolving codes through the redirect chain (C-045); and **§5.3 rule 7 is exercised** — the Index's routing codes are live `alias(kind: route)` bindings, no longer hand-maintained strings (C-046). The `alias` verb's kind set and fold rules are pinned alongside (C-045).
...
*TBD:* the `mint` collision policy under concurrent writers + the fold-verification ritual (OQ-19, workstream E); whether `time` placements should auto-derive from `created` (OQ-18 — non-blocking); a compact binary/columnar projection for very large logs; the log partition interval (OQ-3). *Deferred:* the `place`/`trace`/`query-by-facet` MCP index and the Galaxy UI (build only on observed need, C-021). *Resolved in v0.3 (2026-08-13):* facet minting under the `FACET` root + meta-facet token binding (C-043); global root-uniqueness (C-044); reparent/redirect semantics + the `redirect` alias kind + label/route/redirect fold rules (C-045); rule 7 exercised via live route bindings (C-046). *Resolved in v0.2 (2026-08-12):* the ULID profile, `content_hash` input, `revise` referent, `actor` semantics, TOP `facet:` field, entity-target placements (C-040); the C-015 scheme itself (C-039, closing OQ-14); the identity bridge to canonical documents (C-041).

## 7. Decisions captured

Carried from v0.1/v0.2: **C-022** (node file), **C-023** (event log; interval TBD), **C-024** (log-canonical), **C-025** (supersede + commit granularity; boundary = the git commit per C-037), **C-026** (facet registry + vocabularies), **C-027** (allocation grammar vs style), **C-039** (C-015 confirmed), **C-040** (format pins), **C-041** (identity bridge), **C-042** (canonical corpus minted). New in v0.3 (recorded in Governance Protocol — Claude Series v0.10):

- **C-043** — **Facets are minted concepts.** Each facet is a `TOP-…` minted under the root `FACET` in the meta-facet (token `facet`); the event-field token binds as an `alias` (`kind: route`). The §5.1 table becomes a derived projection of the store. New facets are vocabulary extensions (mint + token binding), not schema edits. Existing events unchanged — tokens now resolve to identities.
- **C-044** — **Root-uniqueness is global.** Sibling uniqueness stays scoped to the parent (rule 3), but root segments form one namespace across all facets; full codes are therefore globally unambiguous, and one-grep traces cannot cross-match roots from different facets.
- **C-045** — **Redirect semantics + alias kinds.** Reparent = `mint` (same subject, new code, parent-validated) + `alias` (`kind: redirect`, old code) in one commit; the fold resolves codes through the redirect chain, materializing current canonical codes while the log stays untouched. Alias kinds pinned: `label` (newest label = `title:`; all labels accumulate) / `route` (machine token → identity, incl. `Fractal_<Name>` bindings from v0.3 forward) / `redirect` (moved code marker).
- **C-046** — **Route binding exercised.** The Index's minted domain routes (`KG`, `GOV`, `GBL`, `CTX`) are live `alias(kind: route)` bindings in the store; the Index routing table is their projection. Remaining domain routes (`AST`, `ONT`, `WS`, `PROT`) follow workstream F.

---

**Refresh triggers:** a change to the two-relation core, the canonical-source rule, the verb set, the role vocabulary, the facet-minting or code-allocation rules, the ULID profile, or the hash definition. *(The §5.1 facet table refreshes as a projection whenever a facet is minted — that alone is not a version bump.)*
**Sources:** Fractal Knowledge Path Foundation v0.1; Fractal Node & Event Schema v0.1/v0.2 (superseded); Governance Protocol — Claude v0.4, v0.6, v0.8, v0.9 & v0.10; Decision Register v0.6; Loose-Ends Scan 2026-08-12 (findings 3.3, 3.4, 8.1); this conversation (2026-08-13, workstream D).
**Revision history:** v0.1 (2026-08-07) initial node & event format specification; (2026-08-12) in-place revision at facet-registry acceptance — retroactively recognized as a C-005 violation (scan 1.2). · v0.2 (2026-08-12) reissue under version-bump discipline: C-015 confirmed (C-039); ULID profile, `content_hash`, `revise` referent, `actor` semantics, TOP `facet:`, entity-target placements pinned (C-040); identity bridge (C-041). · v0.3 (2026-08-13) facet-layer completion (workstream D): facets minted under `FACET` (C-043); global root-uniqueness (C-044); redirect semantics + alias-kind fold rules (C-045); route binding exercised (C-046).
