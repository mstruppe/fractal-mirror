...
**Fractal_Navigation_Contract** · **Version:** 0.2 · **Status:** Ratified (2026-08-20, in-conversation per C-033 — Max's "full slate ratified") · **Updated:** 2026-08-20 · **Domain:** KG · **Author:** Claude · **Parent:** Fractal Knowledge Path Foundation v0.1 · **Supersedes:** Fractal_Navigation_Contract_v0.1.md · **Companion:** Fractal Node & Event Schema v0.7 · **Document ID:** DOC-01M0G6VMYPZ2AKSCAF649W8F6M

**Reviewed By:** Claude (AGENT.AI.CLAUDE) — full conformance review, 2026-08-20: every clause re-read against the live store (151 nodes / 977 events, fold verifier green) and the Schema v0.7 companion; the entry rule's `FACET.*` enumeration and the `resolve` handle classes checked against the log's six facet mints and 126 route tokens; the tier/trigger doctrine confirmed current (Tier 1 still Deferred, the sharpened trigger unfired) · **Ratified by:** Max Struppe (owner — C-033 in-conversation; kernel tier, auto-ratified as shipped per C-133)

**Decision status legend:** *Principle* (accepted, load-bearing) · *Working Decision* (accepted) · *Deferred* · *TBD*.

> **Why v0.2 exists (version-bump discipline, C-040).** A **currency-and-review reissue, no read-behaviour change** — the beta-0.7 boundary its vehicle (Max's scope decision, 2026-08-20; the S3-2.2 cure beside the Schema's v0.7). The header gains the **`Reviewed By` field** (C-133's first exercise alongside the Schema), the companion pointer tracks Schema v0.7, §6's Tier-0 scale claim is re-dated with conformance re-checked, and §8's heading records that C-068–C-070 have long been ratified. Every contract clause — entry, hops, trace, ranking, redirect discipline, history, the verb surface — is unchanged.
...
- **Tier 0 — the grep ritual (live today).** Axes = grep `mint` events with `parent: FACET`; children = grep the code prefix in mints; inhabitants = grep the code across log + front-matter, redirect list first (Schema §3.6); ranking by hand. Conformant by inspection at v0.1's ratification scale (67 nodes / 300 events) and **re-checked at the v0.2 review (151 nodes / 977 events, 2026-08-20)**; it remains the fallback at any scale — the contract is *executable by a human reading plain text*, which is the zero-infra promise kept.
...
## 8. Decisions captured (proposed 2026-08-14; ratified per Protocol v0.24 — the heading's "this session" retired at v0.2)
...
**Sources:** Fractal Knowledge Path Foundation v0.1 (§5 efficient tracing, §6 shells); Fractal Node & Event Schema v0.7 (companion — §3.4 weight, §3.6 inverted view, §4.4 runs, §5.2 code shape, §5.3 redirects); Fractal Navigation Contract v0.1 (superseded); verify.py (the fold it replays); the 2026-08-14 conversation (Max's calls: entry as query shape; weight-product ranking with length/recency tie-breaks; dedicated contract doc with Schema linking); the 2026-08-20 review session (the beta-0.7 scope decision + the C-133 ratification, protocol at that session's close).
**Revision history:** v0.1 (2026-08-14) initial read-side contract: entry rule, hop table, trace contract, history verb, read surface, tiers + sharpened trigger (C-068–C-070 proposed). · v0.2 (2026-08-20) **the currency-and-review reissue (the beta-0.7 boundary its vehicle — the S3-2.2 cure beside Schema v0.7):** the `Reviewed By` header field lands (C-133's first exercise); companion pointer to Schema v0.7; §6's Tier-0 conformance re-checked and re-dated at the live scale; §8's heading made honest (C-068–C-070 ratified per Protocol v0.24); no read-behaviour change.
