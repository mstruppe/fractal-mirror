...
**Version:** 0.42 · **Status:** Living (derived projection) · **Updated:** 2026-08-17 · **Domain:** GOV · **Author:** Claude · **Parent:** Fractal Global Context · **Document ID:** DOC-01KZVYPMBGEY2HV35F32631G0C
...
| 10 | Adopt before invent: check the Registry (`Registry/README.md`) and the shelf of specified components (GENESIS §5; here, this overview + the Architecture State) before building any new working tool or projection. | CS §4.10 / C-092, C-113 |
...
| Sources & materials | A source file (PDF, video, dataset) is material, not knowledge: it lives outside the repo; the repo holds the catalog card (URL/content hash) and the extractions. Scholarly shapes per `Fractal_Scholarly_Source_Convention_v0.1` — pinned editions, identifier routes, PER-not-AGENT authors. | C-115 |
...
| Agenda Board | The board is a standing stamped projection of the Local Context's agenda — repo file `Fractal_Agenda_Board.html`, file-first refresh, never authoritative, offered actively by name when open threads accumulate; format canonized as `Fractal_Agenda_Board_Format_v0.1`. | C-047, C-048, C-035, C-089, C-102, C-104 |
...
| Field testing (the proving process) | Every development landing for real use is proven by a field run: a pre-canon fieldnotes ledger appended run by run, triage and ingestion at the next phase boundary, fixes in the generator. Capture layer: `Fractal_Fieldnote_Format_v0.1` + `fieldnote.py`. | C-094, C-100 |
...
- Full open-question ledger: the Decision Register (stamp inside, C-012) — standing open OQ-4, OQ-9, OQ-27, OQ-28, OQ-29, OQ-31, OQ-32, each on a named trigger or an arc; the review-findings ledger (C-058, including S3-2.2 held open and the `CAP1-…` rows) lives there too; pre-canon vision material informs, never governs (OQ-22). Per-item dispositions: the Register's ledgers and their protocols of record.
