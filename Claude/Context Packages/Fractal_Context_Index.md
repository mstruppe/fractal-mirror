...
**Version:** 0.50 · **Status:** Living (derived projection) · **Updated:** 2026-08-20 (the thirtieth session — the beta-0.6 tagging close: the C-130 section contract, the diet, GENESIS v0.15) · **Document ID:** DOC-01KZVYP8MGAH66JSVDDPNZ1PTR
...
| **The user-document pair series** (the human's windows, C-122/C-124) | all user halves: **`User Documents/`** at the repo root (C-126) — kernel: Board (C-048/C-128) · Roadmap · Handout; extensions: the **Update Plan pair** + the **Gas pair** (originals in `/Claude/Context Packages/`; the close appends the ledger); the standard: `Fractal_User_Document_Pair_Procedure_v0.2` (Governance Documents) |
...
| **The shipping site** (LIVE since 2026-08-18, C-120) | `Site/` — source of record; the section contract (C-130): Home · Guide · **Concepts** (dictionary + `concepts/` detail tier) · **Blog** (the story; the value model its chapter — `value-model.html` a redirect) · **`labs.html`** (the Research Labs, born empty; the KNet name waits on OQ-34) + `legal.html`. Served from the mirror's `gh-pages` through the gate `knet.network`; refreshes push `gh-pages`, pull-before-push (S5-8.2) |
...
| Knowledge Network Foundation **v0.4** (living concept, Draft — non-binding vision tier, C-086; the network stack; §10 the descent, §11 the serving layer, per v0.57) | GBL | `/Claude/Architecture/Concepts/Knowledge Network/` |
| **Value Model** (living concept, **Draft — non-binding vision tier**, C-086 third member, per v0.57; three branches + the horizon + the token-economics constraint; public projection `Site/value-model.html`) | GBL | `/Claude/Architecture/Concepts/Value Model/` |
| Workspace Foundation **v0.2** (living concept, Draft — C-086 second member; the WS forge of record; first build slice `Workbench/`) | WS | `/Claude/Architecture/Concepts/Workspace/` |
| **Interface Foundation v0.5** (living concept, Draft — C-086 fourth member; directions-never-context; §8 PR + the transport; §9 the place; §10 the two connection modes — live in the workspace, fixed at the seam) | GBL | `/Claude/Architecture/Concepts/Interface/` |
...
| Genesis Protocol (`GENESIS.md` **v0.15**, stable root filename; canonical, DOC-minted, STRICT-guarded — BOOTSTRAP's forward sibling; §3 executable via `genesis.py`; clause + §0 pack-checked, current to C-130 at the beta-0.6 pack) | GOV | repo root (`Desktop/FRACTAL/`) |
...
| **User Document Pair Procedure v0.2** (the pair-series standard — the doctrine, the C-125 takeover gate, the kernel trio, the five-step extension process, the C-126 location law; kernel-shipped; v0.1 frozen history) | GOV | `/Claude/Project Governance/Governance Documents/` |
...
| Loose-Ends Scans #1–#5 (C-058; #5 the first `check_scan.py`-gated member) · Stranger Test #1 · the Flight records (Refinement · Diet pair · Migration-Test · Updatability · Scan-5 + Gas-Metering — the gas table's home) | GOV | `/Claude/Context Packages/Conversations/` |
...
*(Sources narrative retired 2026-08-17, the C-110 diet, first conformant exercise of Fractal_Diet_Procedure_v0.1 — index only, **no disposition changed**; per-session narration lives frozen in the protocol files `Governance Protocol/Fractal_Governance_Protocol_Claude_v<X.Y>.md` and the dated `Conversations/Return_Package_<date>_<topic>.md` — the concrete patterns per Diet §6.1, S5-5.2's cure.)*
