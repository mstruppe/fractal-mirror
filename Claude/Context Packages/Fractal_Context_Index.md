...
**Version:** 0.56 · **Status:** Living (derived projection) · **Updated:** 2026-08-23 (the beta-0.8 tagging close, C-140 — the Upgrade Install Procedure row NEW, the KMP row cured to v0.2 (it had lagged the reissue), the pair-series row gains the two seam-pair windows) · **Document ID:** DOC-01KZVYP8MGAH66JSVDDPNZ1PTR
...
*Paths are relative to `Desktop/Knowledge Network/FRACTAL/`. Rows without a dedicated package resolve to their canonical location or the relevant Architecture State index range. ⚭ = route bound in the store via `alias(kind: route)` (C-046).*
...
| **Interface Foundation v0.7** (living concept, Draft — C-086; directions-never-context; §8 PR + the transport + the security clause, the counterpart parameter closed; §9 the place + the live seam; §11 the connection layer) | GBL | `/Claude/Architecture/Concepts/Interface/` |
| **Model Routing Foundation v0.3** (living concept, **Ratified 2026-08-22 — C-138, the C-086 tier's fifth member**; renamed from Engine Routing at ratification, the C-135 collision cure; the gate-coverage principle, the three-layer hedge, the build-on clause — every allocation an offer) | GBL | `/Claude/Architecture/Concepts/Model Routing/` |
...
| Knowledge Graph Store (live: nodes/ + _events/; DOC nodes, facet layer, route bindings; the nine canonized tools — the store README is the roster, C-050…C-139; postman the ninth) | KG | `/Claude/Knowledge Graph Store/` |
...
| Bootstrap Protocol (`BOOTSTRAP.md`, stable root filename) | GOV | repo root (`Desktop/Knowledge Network/FRACTAL/`) |
| Genesis Protocol (`GENESIS.md`, stable root filename — version stamped inside; canonical, STRICT-guarded — BOOTSTRAP's forward sibling; §3 executable via `genesis.py`; pack-checked at every tagging close) | GOV | repo root (`Desktop/Knowledge Network/FRACTAL/`) |
...
| **Interface Place Format v0.2** (the interface place standard — the connection layer: conscious pairing + the lock-in code; the two faces, public + hidden private; the routing law; the postman its checker half; ratified 2026-08-23, the C-123 line; v0.1 frozen history) | GBL | `/Claude/Project Governance/Governance Documents/` |
...
| **Kernel Migration Procedure v0.2** (the release seam's demand side; C-117 + the C-135 fork and the C-134 manifest read; v0.1 frozen history; the §8 advisory template — instantiated per child since beta-0.8, IF-0016/IF-0017) | GOV | `/Claude/Project Governance/Governance Documents/` |
| **Upgrade Install Procedure v0.0** (the campaign runbook seed, C-140 — the beta-0.8 wave's protocolled game plan; RAM at campaign scale, v0.1 distills from its §7 log; the KMP the law it instantiates) | GOV | `/Claude/Project Governance/Governance Documents/` |
...
| Claude Code client adapter (`CLAUDE.md`, stable root filename; canonical per C-065 — content a C-035 compression of Conversation Settings) | GOV | repo root (`Desktop/Knowledge Network/FRACTAL/`) |
...
| Codex drone adapter (`AGENTS.md`, stable root filename; canonical per C-065 class, installed per C-066 — drone-scoped C-035 compression) | GOV | repo root (`Desktop/Knowledge Network/FRACTAL/`) |
